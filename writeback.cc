   ...
   if (Execution_Lanes[lane_number].wb.valid) {
      ...
      if (PAY.buf[index].checkpoint) {
         ...
         if (PERFECT_BRANCH_PRED) {
            ...
            // VPU
            // VPQ rollback - selective squash with instance counter repair
            if(VPU != nullptr){
               if(!VPU->perf_mode){
                  uint64_t vpq_ptr = VPU->m_vpq_tail;
                  uint64_t target  = PAY.buf[index].vpq_tail_checkpoint;

                  uint64_t num_entries;
                  if(VPU->m_vpq_tail_phase == PAY.buf[index].vpq_tail_phase_checkpoint)
                     num_entries = vpq_ptr - target;
                  else
                     num_entries = vpq_ptr + VPU->m_vpq_size - target;
               ...
               VPU->m_vpq_tail = PAY.buf[index].vpq_tail_checkpoint;;
               VPU->m_vpq_tail_phase = PAY.buf[index].vpq_tail_phase_checkpoint;
            }
