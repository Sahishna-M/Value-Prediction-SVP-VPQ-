...
// VPU
static void set_vp_svp_flags(const char *s) {
    uint64_t vpq_size, oracle_conf, num_index_bits, num_tag_bits, conf_max;
    if(sscanf(s, "%lu,%lu,%lu,%lu,%lu", &vpq_size, &oracle_conf, 
              &num_index_bits, &num_tag_bits, &conf_max) != 5){
        fprintf(stderr, "Incorrect usage of --vp-svp=<VPQsize>,<oracleconf>,<#indexbits>,<#tagbits>,<confmax>\n");
        exit(-1);
    }

    // assigning value to the global variables
    VP_VPQ_SIZE       = vpq_size;
    VP_ORACLE_CONF    = (oracle_conf ? true : false);
    VP_NUM_INDEX_BITS = num_index_bits;
    VP_NUM_TAG_BITS   = num_tag_bits;
    VP_CONF_MAX       = conf_max;
    VP_USE_SVP        = true;
}
...
int main(int argc, char **argv) {
   ...
   parser.option(0, "vp-eligible", 1, [&](const char *s) {
    uint64_t pi, pf, pl;
    if (sscanf(s, "%lu,%lu,%lu", &pi, &pf, &pl) != 3) {
        fprintf(stderr, "Incorrect usage of --vp-eligible=<predINTALU>,<predFPALU>,<predLOAD>\n");
        exit(-1);
    }
    vp_pred_INTALU = (pi ? true : false);
    vp_pred_FPALU  = (pf ? true : false);
    vp_pred_LOAD   = (pl ? true : false);
   });
   ...
   // only one predictor can be selected at a time
   if(VP_USE_SVP && VP_USE_PERF){
      fprintf(stderr, "Error: cannot use both --vp-svp and --vp-perf\n");
      exit(-1);
   }

   // VP_USE_SVP == True 
   if(VP_USE_SVP){
      ((pipeline_t*)s_micro->get_core(0))-> init_vpu(
         VP_VPQ_SIZE,
         VP_NUM_INDEX_BITS,
         VP_NUM_TAG_BITS,
         VP_CONF_MAX,
       ...
       ((pipeline_t*)s_micro->get_core(0))-> init_vpu(
        4096,     // VPQ size still needed even in perf mode
        0,               // num_index_bits (not needed for perf)
        0,               // num_tag_bits (not needed for perf)
        0,               // conf_max (not needed for perf)
        true,            // perf_mode = TRUE
        false            // oracle_conf = false
    );
   }
