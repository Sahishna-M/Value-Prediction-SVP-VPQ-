...
482.sphinx3 accumulated approximately 822,502 VR-1 stall cycles, orders of magnitude higher than any other benchmark, due to high value misprediction rates.
456.hmmer discarded approximately 6.3 million instructions across all recoveries.
The average squash of approximately 255 to 256 instructions per recovery event directly mirrors the baseline AL size of 256, confirming that VR-1 waits until the entire AL is drained before recovering regardless of misprediction severity.
...
Redirect Fetch by rolling the Branch Queue back to the last-good branch
Mark the value-predicted instruction complete in the Active List before resolve() is called
Restore the Renamer: RMT, Free List head, Global Branch Mask, and AL tail pointer
Restore the Load/Store Unit tail pointers from the checkpoint snapshot
Repair the VPQ via backward walk, removing younger entries and decrementing SVP instance counters
Squash the backend by clearing the checkpoint bit from all pipeline stage registers
Rollback the payload buffer for all instructions younger than the mispredicted instruction
Reset SVP confidence for the mispredicting PC to zero
Set recovery coordination flags across pipeline stages
...
Files
FileDescriptionvpu.h / vpu.ccSVP and VPQ implementationrenamer.h / renamer.ccPhysical register file, Active List, Free List, RMT managementrename.ccRename stage: VP eligibility check, SVP query, VPQ allocation, stall logicdispatch.ccDispatch stage: predicted value injection into PRF, ready bit setexecute.ccExecute stage: value misprediction detection, VR-5 triggerwriteback.ccWriteback stage: VP check handling for replayed loadsretire.ccRetire stage: SVP training via VPQ, measurement tallyingsquash.ccVR-1 and VR-5 squash and branch resolve logicpipeline.cc / pipeline.hTop-level pipeline with VP command-line argument integrationECE721_proj_4_team_37.pptxCompetition phase research presentation
