...
// VPU
// VPQ
extern bool vp_pred_INTALU;
extern bool vp_pred_FPALU;
extern bool vp_pred_LOAD;

// SVP 
// VPU SVP configuration
// DECLARING THE GLOBAL VARIABLES 
extern uint64_t VP_VPQ_SIZE;             // number of VPQ entries
extern uint64_t VP_NUM_INDEX_BITS;      // number of SVP index bits → SVP size = 2^(index bits)
extern uint64_t VP_NUM_TAG_BITS;        // number of tag bits (0=no tag, full, or partial)

// confidence and mode selection 
extern uint64_t VP_CONF_MAX;            // confidence threshold to be considered confident
extern bool     VP_ORACLE_CONF;         // 0: real confidence, 1: oracle confidence
extern bool     VP_USE_SVP;             // true if user passed --vp-svp (SVP mode selected)
extern bool VP_USE_PERF;                // true if user passed --vp-perf 
