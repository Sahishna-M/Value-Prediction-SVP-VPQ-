...
void pipeline_t::execute(unsigned int lane_number) {
      ...
      if (IS_MEM_OP(PAY.buf[index].flags)) {
         ...
         if (IS_LOAD(PAY.buf[index].flags)) {
            ...
            if (hit && PAY.buf[index].C_valid){
               ...
               // set the dst reg ready bit ready
               REN->set_ready(PAY.buf[index].C_phys_reg);

               // write the val in PRF
               REN->write(PAY.buf[index].C_phys_reg, PAY.buf[index].C_value.dw);

               // VPU 
               // deposit computed value into VPQ
               if(VPU != nullptr && PAY.buf[index].vp_eligible){
                  VPU->m_vpq[PAY.buf[index].vpq_index].m_vpq_val       = PAY.buf[index].C_value.dw;
                  VPU->m_vpq[PAY.buf[index].vpq_index].m_vpq_val_ready = true;
               }

               // VALUE MISPREDICTION CHECK
               if(PAY.buf[index].vp_predicted && PAY.buf[index].vp_confident){
                  if(PAY.buf[index].C_value.dw != PAY.buf[index].vp_value){
   ...
   if (depth > 0) {
      ...
      if (Execution_Lanes[lane_number].ex[depth - 1].valid) {
         ...
         // FIX_ME #11b BEGIN
            unsigned int m_inst_flags = PAY.buf[index].flags;
            bool m_load = IS_LOAD(m_inst_flags);
            bool m_amo = IS_AMO(m_inst_flags);

            if((PAY.buf[index].C_valid) && (!m_load) && (!m_amo)){

               if(!(PAY.buf[index].vp_predicted && PAY.buf[index].vp_confident)){
               // wakeup dependent instructions in the IQ ---- POSSIBLE ERROR
               IQ.wakeup((PAY.buf[index].C_phys_reg), true);
               }

               // set the dst reg ready bit in PRF 
               REN->set_ready(PAY.buf[index].C_phys_reg);
...
void pipeline_t::load_replay() {
   ...
   if (LSU.load_unstall(cycle, index, value)) {
      ...
      if (PAY.buf[index].C_valid) {
         ...
         // FIX_ME #18a BEGIN
         // load hit -> unstall the instruction 
         // VPU 
         if(!(PAY.buf[index].vp_predicted && PAY.buf[index].vp_confident)){
            // wakeup the dependent instructions 
            IQ.wakeup(PAY.buf[index].C_phys_reg, true);
         }
         // set the dst reg ready bit in the PRF
         REN->set_ready(PAY.buf[index].C_phys_reg);

         // write the dst avlue to the PRF 
         REN->write(PAY.buf[index].C_phys_reg, PAY.buf[index].C_value.dw);
