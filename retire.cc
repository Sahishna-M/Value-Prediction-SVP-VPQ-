...
void pipeline_t::retire(size_t &instret) {
   ...
   // FIX_ME #17a BEGIN
      head_valid = REN->precommit(completed,
                                  exception, load_viol, br_misp, val_misp,
   ...
   // FIX_ME #17a END
    ...
    if (head_valid && completed) { // AL head exists and completed
      ...
      if (!exception && !load_viol) {
         ...
         // VPU 
         // VPU -> training of SVP 
         // SVP TRAINING AT RETIRE 
         if(VPU != nullptr && PAY.buf[PAY.head].vp_eligible && !VPU->perf_mode){

            uint64_t computed_value = VPU->m_vpq[VPU->m_vpq_head].m_vpq_val;
            uint64_t pc             = VPU->m_vpq[VPU->m_vpq_head].m_vpq_pc;

            uint64_t pc_index = (pc >> 2) & (VPU->m_svp_size - 1);
            uint64_t pc_tag   = (pc >> (2 + VPU->m_svp_num_index_bits));

            svp_entry_t *entry = &VPU->m_svp[pc_index];

            bool tag_match = false;
            if(VPU->m_svp_num_tag_bits == 0){
               tag_match = entry->m_svp_valid;
            }
            else{
               uint64_t tag_mask = (1ULL << VPU->m_svp_num_tag_bits) - 1;
               tag_match = entry->m_svp_valid && ((entry->m_svp_tag & tag_mask) == (pc_tag & tag_mask));
            }

            if(tag_match){
               // TRAIN
               int64_t new_stride = (int64_t)computed_value - (int64_t)entry->m_svp_retval;
               if(new_stride == entry->m_svp_stride){
                     if(entry->m_svp_conf < VPU->m_svp_conf_max){
                        entry->m_svp_conf++;
                     }
               }
               else{
                     entry->m_svp_stride = new_stride;
                     entry->m_svp_conf   = 0;
               }
               entry->m_svp_retval    = computed_value;
               entry->m_svp_instcount--;
            }
            else{
               // REPLACE - count in-flight instances
               int64_t instance_count = 0;
               uint64_t vpq_ptr = (VPU->m_vpq_head + 1) % VPU->m_vpq_size;

               while(vpq_ptr != VPU->m_vpq_tail){

                     if(VPU->m_vpq[vpq_ptr].m_vpq_pc == pc){
                        instance_count++;
                     }

                     vpq_ptr = (vpq_ptr + 1) % VPU->m_vpq_size;
               }
               uint64_t tag_mask = (VPU->m_svp_num_tag_bits == 0) ? 0 : (1ULL << VPU->m_svp_num_tag_bits) - 1;
               entry->m_svp_tag       = (VPU->m_svp_num_tag_bits == 0) ? 0 :(pc_tag & tag_mask);
               entry->m_svp_conf      = 0;
               entry->m_svp_retval    = computed_value;
               entry->m_svp_stride    = (int64_t)computed_value;
               entry->m_svp_instcount = instance_count;
               entry->m_svp_valid     = true;
            }
         }

         // VPQ - POP 
         // Advance Head
         if(VPU != nullptr && PAY.buf[PAY.head].vp_eligible){
            // Step-1 : Increment Head Pointer
            VPU->m_vpq_head  = (VPU->m_vpq_head + 1);

            // Step-2 : Check Wrap Around Condition
            if( VPU-> m_vpq_head == VPU->m_vpq_size){
               VPU->m_vpq_head= 0;
               VPU->m_vpq_head_phase = !(VPU->m_vpq_head_phase);  		
	         }
         }
         ...
         if(VPU != nullptr){
            if(!PAY.buf[PAY.head].vp_eligible){
               ...
               if(!PAY.buf[PAY.head].vp_hit){
                     // eligible but no prediction (SVP miss)
                     inc_counter(vpmeas_miss);
               }
               else{
                     bool correct = (PAY.buf[PAY.head].C_value.dw == PAY.buf[PAY.head].vp_value);

                     if(PAY.buf[PAY.head].vp_confident && correct){
                        inc_counter(vpmeas_conf_corr);
                     }
                     else if(PAY.buf[PAY.head].vp_confident && !correct){
         ...
         if (amo || csr) { // Resume the stalled fetch unit after committing a serializing instruction.
            ...
            // VPU
            // VPQ rollback - full squash with instance counter repair
            if(VPU != nullptr){
               if(!VPU->perf_mode){
                  uint64_t vpq_ptr = VPU->m_vpq_tail;

                  uint64_t num_entries;
                  if(VPU->m_vpq_tail_phase == VPU->m_vpq_head_phase)
                     num_entries = vpq_ptr - VPU->m_vpq_head;
                  else
                     num_entries = vpq_ptr + VPU->m_vpq_size - VPU->m_vpq_head;

                  for(uint64_t s = 0; s < num_entries; s++){
                     vpq_ptr = (vpq_ptr == 0) ? VPU->m_vpq_size - 1 : vpq_ptr - 1;
                     uint64_t pc = VPU->m_vpq[vpq_ptr].m_vpq_pc;
                     uint64_t pc_index = (pc >> 2) & (VPU->m_svp_size - 1);
                     uint64_t pc_tag   = (pc >> (2 + VPU->m_svp_num_index_bits));
                     svp_entry_t *entry = &VPU->m_svp[pc_index];
                     bool tag_match = false;
                     ...
                     if(tag_match) entry->m_svp_instcount--;
                  }
               }
               VPU->m_vpq_tail = VPU->m_vpq_head;
               VPU->m_vpq_tail_phase = VPU->m_vpq_head_phase;
            }
         ...
         // VPQ rollback - full squash
         // VPU - always reset VPQ tail, repair instance counters only in SVP mode
            if(VPU != nullptr){
               if(!VPU->perf_mode){
                  uint64_t vpq_ptr = VPU->m_vpq_tail;
