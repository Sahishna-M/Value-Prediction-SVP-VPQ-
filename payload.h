...
typedef struct {
   ...
   // VPU 
   bool  vp_eligible;      // is this instruction eligible
   bool  vp_predicted;     // was a prediction made
   bool  vp_confident;     // was the prediction confident
   uint64_t vp_value;      // the predicted value
   uint64_t vpq_index;     // which VPQ entry was allocated 
   bool vp_hit;            // was it a vp hit

   // checkpoint logic 
   // VPQ checkpoint for branch recovery
   uint64_t vpq_tail_checkpoint;
   bool     vpq_tail_phase_checkpoint;  
} payload_t;
