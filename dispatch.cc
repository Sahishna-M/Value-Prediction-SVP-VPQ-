...
void pipeline_t::dispatch() {
   ...
   // FIX_ME #6 BEGIN
      // stall condition for when there are insufficient AL entries 
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      //       load_flag = IS_LOAD(PAY.buf[index].flags); ------ DONE 
      ...
      //       store_flag = (IS_STORE(PAY.buf[index].flags) && (!PAY.buf[index].split_store || !PAY.buf[index].upper)); --- DONE
      ...
      //       branch_flag = IS_BRANCH(PAY.buf[index].flags);-- DONE
      ...
      //       amo_flag = IS_AMO(PAY.buf[index].flags); -- DONE
      ...
      //       csr_flag = IS_CSR(PAY.buf[index].flags); ------ DONE
      ...
      // VPU
      // VALUE PREDICTION INJECTION
      // (i) If instruction was value predicted and confident:
      //     - write predicted value into PRF
      //     - set ready bit of destination register to TRUE (breaking dependency)
      // If not value predicted: ready bit already cleared above (normal behavior)
      if(VPU != nullptr &&  PAY.buf[index].vp_predicted && PAY.buf[index].vp_confident){

         // write predicted value into PRF
         REN->write(PAY.buf[index].C_phys_reg, PAY.buf[index].vp_value);
         
         // set ready bit to TRUE to break data dependency
